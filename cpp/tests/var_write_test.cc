  ...
  protected:
    ...
    void SetUp() override {
      auto factory = [this]() -> arrow::Result<std::shared_ptr<TableProxy>> {
        ...
        auto float_varcol_desc = casacore::ArrayColumnDesc<casacore::Float>(
          "FLOAT_DATA", 2);
        table_desc.addColumn(float_varcol_desc);
        auto complex_varcol_desc = casacore::ArrayColumnDesc<casacore::Complex>(
          "VAR_COMPLEX", 2);
...
TEST_F(EmptyVariableWriteTest, WritePartialFloatVariableEmptyColumn) {
  const auto & table = table_proxy_.table();
  using CT = casacore::Float;

  {
    // Variable data column, partial domain
    auto dtype = arrow::fixed_size_list(arrow::fixed_size_list(arrow::float32(), 2), 2);
    ASSERT_OK_AND_ASSIGN(auto data,
                          ArrayFromJSON(dtype,
                                        R"([[[0, 1],
                                             [2, 3]],
