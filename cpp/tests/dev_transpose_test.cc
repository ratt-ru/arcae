...
TEST_F(DevTransposeTest, Basic) {
  ...
  auto shape_fut = itp->RunAsync(
    [&, column = column](const TableProxy & tp) {
  ...
  auto copy_fut = part_fut.Then(
    [itp = itp, column = column, cpu_pool = cpu_pool](const DataPartition & result) {
      ...
      auto get_data_gen = arrow::MakeMappedGenerator(std::move(data_chunk_gen),
        [
          itp = itp,
          column = column,
          cpu_pool = cpu_pool
        ](const DataChunk & chunk) -> arrow::Future<Array<Complex>> {
          if(chunk.nDim() == 0) return arrow::Status::Invalid("Zero dimension chunk");
          ...
          if(chunk.nDim() == 1) {
            ...
            read_fut = itp->RunAsync([
              ...
              bytes_read += (
                row_slicer.length().product() *
                section_slicer.length().product())*sizeof(Complex);
              auto col = GetArrayColumn<Complex>(tp.table(), column);
