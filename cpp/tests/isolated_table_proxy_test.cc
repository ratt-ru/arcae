...
TEST_F(IsolatedTableProxyTest, MakeTable) {
  ...
  {
    // Test in sync mode
    ASSERT_OK_AND_ASSIGN(auto nrow, itp->RunSync(
      [](const TableProxy & tp) { return tp.table().nrow(); }));
    EXPECT_EQ(nrow, knrow);
  }
  {
    // Test in async mode
    auto fut = itp->RunAsync([](const TableProxy & tp)
...
TEST_F(IsolatedTableProxyTest, RunAsyncConstAndNonConst) {
  ...
  {
    // Test in sync mode
    ASSERT_OK_AND_ASSIGN(auto cnrow, itp->RunSync(
      [](const TableProxy & tp) { return tp.table().nrow(); }));
...
TEST_F(IsolatedTableProxyTest, GetColumn) {
  ...
  {
    // Test in sync mode
    ASSERT_OK_AND_ASSIGN(auto data, itp->RunSync(
